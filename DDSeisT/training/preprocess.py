...
from model import *
import scipy.signal
from data_reader import Config_DD

def load_unet_model(model_dir, device):
    with tf.keras.utils.custom_object_scope({'UNet': UNet}):
        unet_model = tf.keras.models.load_model(os.path.join(model_dir, 'model'))
    return unet_model
...
class DataPreprocessor:
    ...
    def _data_augmentation(self, event: dict) -> dict:
        ...
        if np.random.random() < self.generate_noise_rate:
            ...
            # print("Event dictionary before _clear_dict_except:", event)
...
class SeismicDataset(Dataset):
    ...
    def update_event(self, idx: int, updated_event: dict) -> None:
        """
        Update the event data at the given index.
        Args:
            idx (int): Index
            updated_event (dict): Updated event data
        """
        updated_event['data'] = updated_event['data'].astype(np.float32)
        updated_event["data"] = np.nan_to_num(updated_event["data"])
        self._dataset[idx % self._dataset_size] = (updated_event, self._dataset[idx % self._dataset_size][1])
    ...
    def __getitem__(self, idx: int) -> Tuple[Any, Any, Any, Any]:
        ...
        # unet_model = load_unet_model(self.args.denoiser_dir, self.args.device)
        ...
        # Denoise the wave data
        # wave_data = event['data']
        # try:
        #     wave_data = wave_data.numpy()
        # except:
        #     wave_data = np.array(wave_data)
        # print(wave_data.shape)
        # nChannels = wave_data.shape[0]
        # for i in range(nChannels):
        #     noised_wave_data = wave_data[i]
        #     _, _, noised_wave_data = scipy.signal.stft(noised_wave_data, fs=Config_DD.fs, nperseg=Config_DD.nperseg, nfft=Config_DD.nfft, boundary='zeros')
        #     noised_wave_data = np.stack([noised_wave_data.real, noised_wave_data.imag], axis=-1)
        #     # Denoise the wave data
        #     denoised_wave = unet_model.predict(np.expand_dims(noised_wave_data, axis=0))
        #     denoised_wave = denoised_wave.squeeze(axis=0)

        #     tmp_denoised_wave_data = denoised_wave[..., 0] + 1j * denoised_wave[..., 1]
        #     t, wave = scipy.signal.istft(tmp_denoised_wave_data, fs=Config_DD.fs, nperseg=Config_DD.nperseg, nfft=Config_DD.nfft, boundary='zeros')
        #     wave_data[i] = wave[:Config_DD.nt]

        # std_data = np.std(wave_data, axis=1, keepdims=True)
        # std_data[std_data == 0] = 1
        # wave_data /= std_data

        # # Update the dataset with denoised data
        ...
        # print(f"Type of inputs: {type(inputs)}, inputs: {inputs}")
